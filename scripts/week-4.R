...
# limit test to first 396 verses (to the end of Uccellini)
cits <- cits[trg_start <= 396]
cit.key <- cits[, paste(trg_start, src, src_start)]

t.cits <- t.cits[trg_start <= 396]
t.cit.key <- t.cits[,paste(trg_start, src, src_start)]

all.tess <- all.tess[trg_start <= 396]
all.tess.key <- all.tess[, paste(trg_start, src, src_start)]
...
# output directory
dir.plot <- file.path("plot", "week-4")

# Tesserae true and false positive results for cutoff 9 & 10
pdf(file=file.path(dir.plot, "tess_precision_1-396.pdf"), width=8, height=4)
...
pdf(file=file.path(dir.plot, "tess_p-r_1-396.pdf"), width=8, height=4)
with(tess.prec, 
...
pdf(file=file.path(dir.plot, "venn_valid_1-396.pdf"), width=6, height=6)
...
pdf(file=file.path(dir.plot, "venn_raw_1-396.pdf"), width=6, height=6)
grid.draw(venn.raw)
dev.off()
...
pdf(file=file.path(dir.plot, "hist_all_tess_1-396.pdf"), width=8, height=4)
...
pdf(file=file.path(dir.plot, "hist_tess_valid_1-396.pdf"), width=8, height=4)
...
pdf(file=file.path(dir.plot, "hist_comm_1-396.pdf"), width=8, height=4)
